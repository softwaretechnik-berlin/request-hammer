...
class SchedulingException(val scheduledOps: Int) extends RuntimeException()

class Scheduler(toleranceMillis: Long)(implicit executor: ExecutionContext) {
  ...
  def run[T](schedule: Schedule[T]): Seq[Future[Timed[Try[T]]]] = {
    ...
    val futureResponses = schedule.operations.toList.map { case ScheduledAsyncOperation(scheduledTime, fun) =>
      ...
      if (scheduledTime.value < currentTime - toleranceMillis) {
        throw new SchedulingException(count)
      }
